...
10. **CLI agent command** — prefer `registry.json` `settings.hub_agent.cliName`
    ...
    that as migration and set `hub_agent.cliName` (do not reintroduce `.cli` as
    the live source of flags). If neither exists, ask which CLI they use
    (e.g. `claude`, `codex`, `grok`), confirm with `which`, ensure `cli.json`
    has that catalog entry, and set `settings.hub_agent.cliName`.
