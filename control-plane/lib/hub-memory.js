...
function deriveHubRuntimePrompt(hub) {
  ...
  return [
    ...
    '',
    'To wake yourself or an agent later without blocking this turn (no heartbeat daemon):',
    '',
    '```bash',
    'scripts/defer.sh --to <slug|hub> --from hub --in 15m --subject "wake: …" --body "…" [--once-key KEY]',
    '```',
    '',
    'Fires once via OS timer → inbox mail with `kind: defer-wake`. Treat that mail like normal inbox work; reschedule with the same `--once-key` for poll loops. Cancel: `scripts/defer.sh --cancel <id>`. List: `scripts/defer.sh --list`. Spec: `docs/DEFERRED-WAKE.md`.',
    '',
