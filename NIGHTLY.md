...
3. Prune old files under `*/archive/` directories (`scripts/prune-archives.sh`,
   default retention 15 days from `settings.tuning.archive.retention_days`).
4. For each project in `registry.json`, run `git log --since=midnight` to
   detect the day's commits.
5. For each project with activity, refresh its root `sitemap.md` and add a
...
6. If any work was done this run, add a hub journal entry under `journal/`.
